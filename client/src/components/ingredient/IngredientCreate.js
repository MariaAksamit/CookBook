import React, { useState, useEffect } from "react";
import { Modal, Button, Form } from 'react-bootstrap';
import Icon from "@mdi/react";
import { mdiPlus } from "@mdi/js";
import styles from "../../styles/recipeList.module.css";

export default function IngredientCreate ({ handleShowModal, ingredientList }) {
  const [isModalShown, setShow] = useState(false);
  const [ingredientCall, setIngredientCall] = useState({ state: "pending" });
  const [duplicateError, setDuplicateError] = useState(null);
  const [ingredientName, setIngredientName] = useState('');

  const handleCloseModal = () => {
    setShow(false);
    setDuplicateError(null); // Resetuje chybovú správu duplicity pri zatváraní modálneho okna
  };

  const handleOpenModal = () => {
    handleShowModal();
    setShow(true);
  }
  
  useEffect(() => {
    if (isModalShown) handleOpenModal();
  }, [isModalShown]);

  const handleCreateIngredient = async (e) => {
    try {
      e.preventDefault();
      e.stopPropagation();
    
      // Skontroluje duplicity
      const isDuplicate = checkForDuplicate(ingredientName);

      if (isDuplicate) {
        setDuplicateError("Název suroviny již existuje. Zvolte jiný název.");
      return;
      } 
      
      // Kontrola počtu znakov v názve suroviny (2-50) a nechcených znakov
      const validCharactersRegex = /^[a-zA-Z0-9\s-,áäéěíýóúůčďňšťžľĺřŕÁÉÍÝÓÚŮČĎŇŠŤŽĽĹŘŔ]+$/; // Povolené sú písmená, číslice, medzery a pomlčky

      // Kontrola počtu znakov v názve suroviny (2-50)
      if (ingredientName.length < 2 || ingredientName.length > 25 || !validCharactersRegex.test(ingredientName)) {
        setDuplicateError("Zadejte název suroviny o délce 2 až 25 povolených znaků (písmena, číslice, mezery a pomlčky).");
      return;
      }

      const newIngredient = {
        name: ingredientName,
      };
    
      const response = await fetch("/ingredient/create", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(newIngredient),
      });
    
      if (response.ok) {
        setShow(false);
      } else {
        const errorData = await response.json();
        setIngredientCall({ state: "error", error: errorData });
      }
      } catch (error) {
        setIngredientCall({ state: "error", error: error.message });
      } finally {
        //window.location.reload();
    }
  };
  
  // Funkcia na kontrolu duplicity
  const checkForDuplicate = (name) => {
    const existingIngredients = ingredientList.map((item) =>
      item.name.toLowerCase()
    );
    return existingIngredients.includes(name.toLowerCase());
  };

return (
<>
  <Button 
    variant="success"
    onClick={handleOpenModal}>
    <Icon path={mdiPlus} size={1} />
      Create
  </Button>

  <Modal show={isModalShown} onHide={handleCloseModal}>
    <Modal.Header closeButton>
      <Modal.Title className={styles.colorText}> Vytvoření nové suroviny: </Modal.Title>
    </Modal.Header>

    <Modal.Body>
      <Form.Group> 
        <Form.Control 
          type="text" 
          placeholder="Nová surovina"
          value={ingredientName}
          onChange={(e) => {
            setIngredientName(e.target.value);
            setDuplicateError(null); // Resetuje chybovú správu duplicity po zmene názvu
          }}
          required
        /> 
          {duplicateError && (
            <Form.Text className="text-danger"> {duplicateError} </Form.Text>
          )}
      </Form.Group>
    </Modal.Body>
        
    <Modal.Footer>
      <Button 
        variant="primary"
        style={{ marginLeft: "8px" }}
        onClick={handleCreateIngredient} 
      >
        Uložit
      </Button>
      <Button 
        variant="secondary-outline"
        style={{ marginLeft: "8px"}}
        onClick={handleCloseModal} 
      >
        Zavřít
      </Button>  
    </Modal.Footer>
  </Modal>
</>
);
};